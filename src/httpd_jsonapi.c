...
static json_object *
genre_to_json(struct db_browse_info *genre)
{
  json_object *item;
  ...
  item = json_object_new_object();
  safe_json_add_string(item, "name", genre->string);
  safe_json_add_string(item, "name_sort", genre->sortstring);
  json_object_object_add(item, "track_count", json_object_new_int(genre->count));
  ...
  json_object_object_add(item, "length", json_object_new_int64((genre->length / 1000)));

  return item;
}

// TODO - fold genre/composer_to_json
static json_object *
composer_to_json(const struct db_browse_info *composer)
{
  json_object *item;
  ...
  item = json_object_new_object();
  safe_json_add_string(item, "name", composer->string);
  safe_json_add_string(item, "name_sort", composer->sortstring);
  json_object_object_add(item, "track_count", json_object_new_int(composer->count));
  json_object_object_add(item, "album_count", json_object_new_int(composer->album_count));
  json_object_object_add(item, "artist_count", json_object_new_int(composer->artist_count));
  json_object_object_add(item, "length", json_object_new_int64((composer->length / 1000)));
...
{
  struct db_browse_info browse_info;
  json_object *item;
  int ret;
  ...
  while (((ret = db_query_fetch_browse(query_params, &browse_info)) == 0) && (browse_info.string))
    {
      item = genre_to_json(&browse_info);
      if (!item)
	{
	  ret = -1;
	  goto error;
...
fetch_composers(struct query_params *query_params, json_object *items, int *total)
{
  ...
  while (((ret = db_query_fetch_browse(query_params, &browse_info)) == 0) && (browse_info.string))
    {
      item = composer_to_json(&browse_info);
      if (!item)
	{
	  ret = -1;
	  goto error;
...
jsonapi_reply_library_genres(struct httpd_request *hreq)
{
  ...
  query_params.type = Q_BROWSE_GENRES_WITH_COUNT;
  query_params.idx_type = I_NONE;
...
jsonapi_reply_library_composers(struct httpd_request *hreq)
{
  ...
  query_params.type = Q_BROWSE_COMPOSERS_WITH_COUNT;
